...
func (cfg *config) checkOneLeader() int {
	for iters := 0; iters < 10; iters++ {
		...
		for i := 0; i < cfg.n; i++ {
			if cfg.connected[i] {
				//fmt.Printf("raft[%d]' is %d--",i,cfg.rafts[i].Getstate())
...
func (cfg *config) checkNoLeader() {
	for i := 0; i < cfg.n; i++ {
		if cfg.connected[i] {
			...
			//fmt.Printf("Raft[%d]'s state is %d\n",i,cfg.rafts[i].Getstate())
			if is_leader {
...
func (cfg *config) nCommitted(index int) (int, interface{}) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		...
		cfg.mu.Unlock()
		//fmt.Printf("now the Raft[%d]'s logs is ",i)
		...
		if index != -1 {
			...
			for time.Since(t1).Seconds() < 2 {
				...
				if nd > 0 && nd >= expectedServers {
					// committed
					//fmt.Printf("There are %d raft commited %d\n",nd,cmd1)
